...
export class CfdChart extends LitElement {
  ...
  render() {
    // Los snapshots pueden llegar como objeto {turn: snap} o como array (RTDB convierte
    // claves numéricas en lista, con hueco null en el índice 0). Normalizamos a lista.
    const raw = this.snapshots || {};
    const list = (Array.isArray(raw) ? raw : Object.values(raw))
      .filter((s) => s && s.perColumn)
    ...
    // Suma total por snapshot para escalar Y.
    const totalAt = (s) => cols.reduce((sum, c) => sum + (s.perColumn?.[c.id] || 0), 0);
    const maxStack = Math.max(1, ...list.map(totalAt));

    const x = (i) => M.l + (list.length === 1 ? innerW / 2 : (i / (list.length - 1)) * innerW);
    const y = (v) => M.t + innerH - (v / maxStack) * innerH;

    // Cumulativos por columna y snapshot.
    const cum = list.map((s) => {
      let acc = 0;
      return cols.map((c) => { const lo = acc; acc += (s.perColumn?.[c.id] || 0); return { lo, hi: acc }; });
    });

    const areas = cols.map((c, k) => {
      const top = list.map((s, i) => `${x(i).toFixed(1)},${y(cum[i][k].hi).toFixed(1)}`);
      const bottom = list.map((s, i) => `${x(i).toFixed(1)},${y(cum[i][k].lo).toFixed(1)}`).reverse();
    ...
    return html`
      <div class="chart-wrap">
        <svg viewBox="0 0 ${W} ${H}" width="100%" role="img" aria-label="Diagrama de flujo acumulado">
          ...
          ${areas.map((a) => svg`<polygon points=${a.points} fill=${a.color} opacity="0.85"></polygon>`)}
          ${list.map((s, i) => svg`<text x=${x(i)} y=${H - 10} text-anchor="middle" fill="#6f819a" font-size="10">T${s.turn ?? i}</text>`)}
          ${cols.map((c, k) => svg`
