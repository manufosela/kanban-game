...
export async function startGame(board, { wipEnabled = false, rondas = 2, ciclos = 5, timeLimitMinutes = null, pauseBetweenRounds = false } = {}) {
  ...
  if (wipEnabled) {
    ...
    const analysisId = R.anchors(cols).id.analysis; // Refinement: buffer de entrada, siempre sin WIP
    const updates = {};
    cols.forEach((c) => {
      const val = c.id === analysisId ? null : wip[c.id];
      if (c.id === analysisId || val != null) {
        c.wipLimit = val ?? null;
        updates[`boards/${board.id}/columns/${c.id}/wipLimit`] = c.wipLimit;
      }
    });
...
export function botAction(state) {
  ...
  if (step === STEP.DEVS) {
    ...
    // Los bots esperan a que cada humano haya elegido (cogido) una tarea o ya actuado.
